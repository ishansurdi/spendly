...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO transactions (transaction_id, user_id, type, amount, reason, category, timestamp, day, previous_balance, after_balance)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssdssssdd", $transaction_id, $uid, $type, $amount, $reason, $category, $timestamp, $day, $current_balance, $new_balance);
    if (!$stmt->execute()) {
